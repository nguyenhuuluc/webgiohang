...
function showItem()
{
        var showout = document.querySelectorAll('.miniCart-body .miniCart-item');
        var t = "";
...
$(document).on('click', '.add', function(){
    ...
    for(let i = 0; i < carts.length; i++){
        if(name == carts[i][1])
        {
            ...
            console.log(carts[i][3]);

            carts[i][4] = parseInt(carts[i][3] * carts[i][2]);
            $(this).parent().parent().parent().parent().parent().siblings('.miniCart-footer').children('miniCart-total').children('span.miniCart-price').text(carts[i][4] + "₫");
            totalItem()
            showItem()
            addQuantity()
            sessionStorage.setItem("carts", JSON.stringify(carts))
